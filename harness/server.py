...
@app.get("/catalogue")
async def catalogue():
    """Proxy the mock target's model catalogue (names, which are usable, current).
    Returns empty for a target that has no catalogue endpoint (e.g. a real
    external API), so callers can just hide the selector."""
    try:
        r = _client.get(f"{mapper.get_mock_url().rstrip('/')}/models", timeout=10)
        return r.json()
...
async def switch_model(request: Request):
    """Proxy a live model switch to the mock target."""
    raw = await request.body()
    try:
        r = _client.post(
            f"{mapper.get_mock_url().rstrip('/')}/model",
            content=raw, headers={"Content-Type": "application/json"}, timeout=10,
        )
...
async def set_risk(request: Request):
    """Proxy a session-risk update to the mock target's governance rung."""
    raw = await request.body()
    try:
        r = _client.post(
            f"{mapper.get_mock_url().rstrip('/')}/risk",
            content=raw, headers={"Content-Type": "application/json"}, timeout=10,
        )
        return JSONResponse(r.json(), status_code=r.status_code)
    except httpx.RequestError as e:
...
@app.post("/governance")
async def toggle_governance(request: Request):
    """Proxy a runtime governance on/off toggle to the mock target."""
    raw = await request.body()
    try:
        r = _client.post(
            f"{mapper.get_mock_url().rstrip('/')}/governance",
            content=raw, headers={"Content-Type": "application/json"}, timeout=10,
        )
        return JSONResponse(r.json(), status_code=r.status_code)
    except httpx.RequestError as e:
...
@app.post("/prompt")
async def switch_prompt(request: Request):
    """Proxy a live system-prompt switch (GUI toggle: neutral <-> hardened)."""
    raw = await request.body()
    try:
        r = _client.post(
            f"{mapper.get_mock_url().rstrip('/')}/prompt",
            content=raw, headers={"Content-Type": "application/json"}, timeout=10,
        )
        return JSONResponse(r.json(), status_code=r.status_code)
    except httpx.RequestError as e:
...
async def get_policy():
    """Proxy the target's Cedar policy file (raw text, for the GUI to open in a tab)."""
    from fastapi.responses import PlainTextResponse
    try:
        r = _client.get(f"{mapper.get_mock_url().rstrip('/')}/policy", timeout=10)
...
async def health():
    ...
    # Expose both endpoints, and whether switching between them is meaningful.
    # When mock and real resolve to the same host:port (e.g. a self-contained
    # mock target), the real/mock switch is a no-op, so a client can hide it.
    # Compare host:port only — get_api_url appends the request path while
    # get_mock_url does not, so the raw strings never match even for one target.
    try:
        mock_url = mapper.get_mock_url()
    except Exception:
        mock_url = None
    try:
        real_url = mapper.get_api_url()
    except Exception:
        real_url = None
    ...
    if mock_url and real_url:
        backend_switchable = _netloc(mock_url) != _netloc(real_url)
    else:
        backend_switchable = True  # unknown -> keep the switch (fail open)

    _gov = PROFILE.get("mock", {}).get("governance", {})
    _prompts = PROFILE.get("mock", {}).get("prompts", {}) or {}
    _default_prompt = PROFILE.get("mock", {}).get("default_prompt")
    return {"status": "ok", "target": TARGET_NAME, "display_name": DISPLAY_NAME, "mapper": mapper.name, "backend": BACKEND, "api_url": _api_url, "mock_url": mock_url, "real_url": real_url, "backend_switchable": backend_switchable, "cert_status": cert_status, "firewall_enabled": FIREWALL_ENABLED, "governance_configured": bool(_gov), "governance_enabled": bool(_gov.get("enabled")), "governance_policy": _gov.get("policy"), "prompt_switchable": len(_prompts) >= 2, "prompt_available": list(_prompts.keys()), "prompt_default": _default_prompt, "profile": PROFILE_PATH, "intel_dir": INTEL_DIR}
