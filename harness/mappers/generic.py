...
class GenericMapper(BaseMapper):
    ...
    def has_message_field(self, body: dict) -> bool:
        """True if the request carries a prompt field, even an empty one.

        Distinguishes "the scanner sent an empty prompt", which is a legitimate
        edge-case probe and should be answered, from "this body has no prompt
        field at all", which is an integration mistake worth a 400.
        """
        if self._prompt_field:
            probe = resolve_dot_path(body, self._prompt_field)
    ...
    def build_mock_response(
        ...
        if isinstance(result, dict):
            ...
            # Preserve trusted tool events through the formatter, the
            # same contract ExampleMapper follows: present only when tool capture
            # ran, an explicit [] meaning a complete empty capture.
            if "events" in kwargs:
                result["events"] = kwargs["events"]
        return result
